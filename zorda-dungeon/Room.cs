					...
					if (i == 0 || i == 6)
					{
						if (k == 4)
						{
							if (i == 0 && checkRoomDirection(RoomDirection.Up))
							{
								...
								spriteBatch.Draw(this.floorSpr, new Rectangle(366, 15, 64, 64), this.floorColor);
								continue;
							}
							if (i == 6 && checkRoomDirection(RoomDirection.Down))
							{
								...
								spriteBatch.Draw(this.floorSpr, new Rectangle(366, 399, 64, 64), this.floorColor);
								continue;
							}
						}
						walls[k][i] = new Entity(this.blockSpr, this.blockSpr, new Vector2(110 + 64 * k, 15 + 64 * i), this.wallColor);
						walls[k][i].Draw(spriteBatch);
					}
					else
					{
						spriteBatch.Draw(this.floorSpr, new Rectangle(110 + 64 * k, 15 + 64 * i, 64, 64), this.floorColor);

						if (!(i == 3 && checkRoomDirection(RoomDirection.Left)))
						{
							walls[1][i] = new Entity(this.blockSpr, this.blockSpr, new Vector2(110, 15 + 64 * i), this.wallColor);
							walls[1][i].Draw(spriteBatch);
						}
						else
						{
							walls[0][i] = new Door(this.blockSpr, this.blockSpr, new Vector2(44, 15 + 64 * i), RoomDirection.Left);
							spriteBatch.Draw(this.floorSpr, new Rectangle(110, 207, 64, 64), floorColor);
						}

						if (!(i == 3 && checkRoomDirection(RoomDirection.Right)))
						{
							walls[9][i] = new Entity(this.blockSpr, this.blockSpr, new Vector2(622, 15 + 64 * i), this.wallColor);
							walls[9][i].Draw(spriteBatch);
						}
						else
						{
							walls[10][i] = new Door(this.blockSpr, this.blockSpr, new Vector2(686, 15 + 64 * i), RoomDirection.Right);
							spriteBatch.Draw(this.floorSpr, new Rectangle(622, 207, 64, 64), this.floorColor);
						}
					}
				}
			}
